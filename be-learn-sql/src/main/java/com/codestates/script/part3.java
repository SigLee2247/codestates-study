...
public class part3 {
  ...
  */
  public static final String PART3_2_2 = "SELECT user.name,email,role.name FROM user LEFT JOIN role ON user.roleId = role.id ;";
  ...
  */
  public static final String PART3_2_5 = "SELECT title FROM content WHERE id = (SELECT id  FROM user WHERE name = 'minsanggu');";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 3-2-6. minsanggu이 작성한 content의 category name을 찾기위한 SQL을 작성해주세요.
  */
  public static final String PART3_2_6 = "SELECT category.name FROM category " +
          "WHERE category.id in (SELECT categoryId  FROM content_category " +
          "WHERE contentId in (SELECT content.id FROM content " +
          "WHERE userId in (SELECT user.id FROM user WHERE user.name = 'minsanggu')));";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 3-2-7. category의 name이 java인 content의 title, body, created_at을 찾기위한 SQL을 작성해주세요.
  */
  public static final String PART3_2_7 = "SELECT title, body, created_at FROM content " +
          "WHERE id in (SELECT contentId FROM content_category " +
  ...
  */
  public static final String PART3_2_8 = "SELECT content.title,content.body,content.created_at,user.name FROM user " +
          "LEFT JOIN content ON user.id = userId " +
          "LEFT JOIN content_category ON content.id = contentId " +
  ...
  */
  public static final String PART3_2_9 = "SELECT COUNT(*) AS ContentCount FROM content WHERE userId = (SELECT id FROM user WHERE name = 'teawoongna'); ";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 3-2-10. 각 user(컬럼명: name)가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
  */
  public static final String PART3_2_10 = "SELECT user.name AS name, count(content.userId) AS ContentCount " +
          "FROM user LEFT JOIN content ON user.id = userId " +
          "GROUP BY user.name ";
}
